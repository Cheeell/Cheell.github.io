      ...
      <div style={{maxWidth:1300,margin:"0 auto",padding:"0 32px"}}>
        ...
        <Reveal delay={0.1}>
          <div style={{marginTop:48,padding:"36px",background:"var(--s1)",border:"1px solid var(--bs)",borderRadius:14}}>
            <div style={{fontFamily:"'DM Mono',monospace",fontSize:11,letterSpacing:"0.08em",textTransform:"uppercase",color:"var(--td)",marginBottom:10}}>What's included in your £99 strategy</div>
...
/* ─── CONFIG ──────────────────────────────────────────────────
   These are read from your .env file automatically by Vite.
   Make sure your .env contains ALL THREE:

   VITE_SUPABASE_URL=https://xxxxxxxxxxxx.supabase.co
   VITE_SUPABASE_ANON_KEY=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...
   VITE_STRIPE_PAYMENT_URL=https://buy.stripe.com/...
...
const Questionnaire = ({ setPage }) => {
  ...
  const submit = async () => {
    ...
    try {
      /* 0. Guard — catch missing env vars before any network call */
      ...
      /* 1. Save survey data to Supabase */
      const surveyData = { ...answers };
      const payload = {
        email: answers.email,
        business_name: answers.businessName || null,
        industry: answers.industry || null,
        business_type: (answers.businessType || "").split(" — ")[0] || null,
      ...
      const res = await fetch(`${SUPABASE_URL}/rest/v1/emails`, {
        ...
        headers: {
          ...
          // "return=minimal" → empty 204 body (no JSON parse needed)
          // "resolution=merge-duplicates" → upsert on duplicate email
      ...
      if (!res.ok) {
        // Only parse body if there is content (status 400/422/500 etc.)
      ...
      // 201 / 204 — no body to parse, continue straight to Stripe

      /* 2. Cache data in sessionStorage so it survives Stripe redirect */
      sessionStorage.setItem("pendingEmail", answers.email);
      sessionStorage.setItem("pendingBusiness", JSON.stringify(answers));

      /* 3. Build Stripe URL and redirect */
      // Stripe Payment Links only support prefilled_email as a query param.
      // success_url / cancel_url must be set in the Stripe Dashboard under
      // Payment Link → After payment → Redirect to your website
      // Set your success URL to: https://yourdomain.com/?payment=success
      const stripeUrl = new URL(STRIPE_PAYMENT_URL);
      stripeUrl.searchParams.set("prefilled_email", answers.email || "");
  ...
  /* ── Payment success screen (returned from Stripe) ── */
  if (paymentSuccess) {
    const cachedEmail = sessionStorage.getItem("pendingEmail") || "—";
    const cachedBiz = (() => { try { return JSON.parse(sessionStorage.getItem("pendingBusiness") || "{}"); } catch { return {}; } })();
    return (
      <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",padding:"80px 24px 40px"}}>
        <div style={{textAlign:"center",maxWidth:500,animation:"popIn .4s cubic-bezier(.34,1.56,.64,1)"}}>
          ...
          <h2 style={{fontFamily:"'EB Garamond',serif",fontSize:42,fontWeight:400,marginBottom:14}}>Payment confirmed</h2>
          <p style={{fontSize:15,color:"var(--tm)",lineHeight:1.72,marginBottom:28}}>
            Thank you! Your payment was successful. Marta will personally review your strategy and deliver it to your inbox within <strong style={{color:"var(--t)"}}>5–7 business days</strong>.
          </p>
          <div style={{background:"var(--s1)",border:"1px solid var(--bs)",borderRadius:12,padding:24,textAlign:"left",marginBottom:24}}>
            {[
              ["Email", cachedEmail],
              ["Business", cachedBiz.businessName || "—"],
              ["Industry", cachedBiz.industry || "—"],
              ["Amount paid", "£50.00"],
            ...
            <div style={{display:"flex",justifyContent:"space-between",padding:"9px 0",fontSize:13}}>
              <span style={{color:"var(--tm)"}}>Payment status</span>
              <span style={{color:"var(--green)",fontWeight:500}}>Completed ✓</span>
            </div>
          </div>
          <div style={{background:"rgba(34,197,94,.06)",border:"1px solid rgba(34,197,94,.18)",borderRadius:10,padding:"16px 20px",marginBottom:28,textAlign:"left"}}>
            <div style={{fontSize:13,color:"var(--green)",fontWeight:500,marginBottom:8}}>What happens next</div>
            {["AI analyses your business data","Marta personally reviews the strategy","Polished PDF report is created","Strategy delivered to your email"].map((s,i) => (
              <div key={i} style={{fontSize:13,color:"var(--tm)",padding:"4px 0",display:"flex",gap:8}}>
                <span style={{color:"var(--green)",flexShrink:0}}>{i+1}.</span>{s}
              </div>
            ))}
          </div>
          <button onClick={()=>{setPage("home");window.scrollTo(0,0);}}
            style={{padding:"11px 24px",border:"1px solid var(--bs)",borderRadius:8,background:"none",color:"var(--tm)",fontSize:14,fontFamily:"'DM Sans',sans-serif",cursor:"pointer",transition:"all .15s"}}
    ...
    <div style={{minHeight:"100vh",display:"flex",paddingTop:56}}>
      ...
      <main className="q-main" ref={contentRef}>
        <div className="q-form">
          ...
          {/* Legal consent — only show on last step */}
          {step === STEPS.length - 1 && (
            <div style={{marginBottom:20,padding:"16px",background:"var(--s2)",border:"1px solid var(--bs)",borderRadius:10}}>
              <label style={{display:"flex",alignItems:"flex-start",gap:10,cursor:"pointer"}}>
                <div onClick={()=>setLegalConsent(p=>!p)} style={{width:18,height:18,border:`1px solid ${legalConsent?"var(--green)":"var(--bs)"}`,borderRadius:4,flexShrink:0,marginTop:1,display:"flex",alignItems:"center",justifyContent:"center",background:legalConsent?"var(--green)":"transparent",transition:"all .15s",fontSize:11,color:"black",fontWeight:700,cursor:"pointer"}}>
                  {legalConsent?"✓":""}
                </div>
                <span style={{fontSize:13,color:"var(--tm)",lineHeight:1.55}}>
                  I agree to the{" "}
                  <a href="/terms.html" target="_blank" rel="noopener noreferrer" style={{color:"var(--t)",textDecoration:"underline"}}>Terms & Conditions</a>,{" "}
                  <a href="/privacy.html" target="_blank" rel="noopener noreferrer" style={{color:"var(--t)",textDecoration:"underline"}}>Privacy Policy</a>{" "}and{" "}
                  <a href="/cookie.html" target="_blank" rel="noopener noreferrer" style={{color:"var(--t)",textDecoration:"underline"}}>Cookie Policy</a>.
                  You will be redirected to Stripe to complete your £50 payment.
                </span>
              </label>
            </div>
          )}
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:16}}>
            ...
            <button onClick={next}
              disabled={!isValid()||submitting||(step===STEPS.length-1&&!legalConsent)}
              style={{display:"flex",alignItems:"center",gap:8,padding:"11px 24px",border:"none",borderRadius:8,background:"white",color:"black",fontSize:14,fontWeight:500,fontFamily:"'DM Sans',sans-serif",cursor:(isValid()&&!submitting&&(step<STEPS.length-1||legalConsent))?"pointer":"not-allowed",opacity:(isValid()&&!submitting&&(step<STEPS.length-1||legalConsent))?1:.35,transition:"opacity .15s, transform .15s"}}
              ...
              {submitting?"Redirecting to Stripe…":step===STEPS.length-1?"Pay £50 → Stripe":"Continue →"}
            </button>
          </div>
